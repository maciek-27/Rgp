...
void * ServeConnnection(void * _conn)
{
	...
	cerr << "in thread for conn fd: "<< conn->fd << endl;
...
__Connection::__Connection(int _fd)
{
	...
	cerr << "Accepted connection; fd = " << fd << endl;
	iF = fdopen(fd,"r");
	oF = fdopen(fd,"w");
...
void Server::MessageEvent(const UserInfo& info,
						  const std::wstring& msg)throw()
{
	for(set<ProgEntry>::iterator i = allprogs.begin(); i != allprogs.end();
		i++) {
		Scr::Demo &target = const_cast<ProgEntry &>((*i)).GetEntry();
		target.MessageEvent(info, msg);
	}
}

void Server::JoinEvent(const UserInfo& info)throw()
{
	s.nicks.push_back(info.userName);
	for(set<ProgEntry>::iterator i = allprogs.begin(); i != allprogs.end();
		i++) {
		Scr::Demo &target = const_cast<ProgEntry &>((*i)).GetEntry();
		target.JoinEvent(info);
	}
}

void Server::LeaveEvent(const UserInfo& info)throw()
{
	for(set<ProgEntry>::iterator i = allprogs.begin(); i != allprogs.end();
		i++) {
		Scr::Demo &target = const_cast<ProgEntry &>((*i)).GetEntry();
		if( &target.userInfo != &info)
			target.LeaveEvent(info);
	}
	s.nicks.remove(info.userName);
}
