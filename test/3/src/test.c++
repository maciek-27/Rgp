  ...
  public:
      ...
      for (int i = 0 ; i!= columns; ++i)
	{
	  if (VG[i]->GetNumElements()<rows)
	    {
	      num_elem++;
	      VG[i]->AddWidget(widget, stretchFactor);
	      return;
	    }
	}
    ...
    void SetNumColumns(int col)throw()
    {
      ...
      if (columns>col)
	{
	  rows=num_elem/columns;
	  if (num_elem>rows*columns)
	    rows++;
...
void ProcessUI::PrintStack()
{
  ...
  strst.str("");
  strst<< "cpu: "<< 
    (float(utime+stime)/ sysconf(_SC_CLK_TCK)) << "s U, "<<

    (float(utime+stime)/ sysconf(_SC_CLK_TCK)) << "s K, "<<
