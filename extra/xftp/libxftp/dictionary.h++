...
namespace xftp
{
	...
	private:
        ...
        static int strcmp(const C * s1, const C* s2) 
		{
            int i=0;
            do
			{
        ...
        void StringCopy(C * dest, const C* src)
		{
			do
			{
				*dest=*src;
...
#define record vector[ vmark( static_cast<size_t>(name[current]) ) ]
		 ...
		 */
		t_name_record * tree_add (const C * name);
		 ...
		 *  character of name)
		 */
		t_name_record * tree_partial_find (const C * name,
		 ...
		 *  character of name)
		 */
		t_name_record * tree_find (const C * name,
	...
	public:
		...
		public:
			...
			bool operator<(const iterator & it) 
			{
				...
				else
				{
					if (it._node==NULL)//everything is < end()
						...
						return strcmp (_node->name, it._node->name)<0;
					else
			...
			/*!
			 * \return key associated w/ this iterator
			 */
		 ...
		 * \param n new element
		 */
		void insert (const C * name, const T & n)
		{
		 ...
		 * \note consider using contains(const C *), when need only to check,
		 * whenever element  exists.
		 */
		iterator find (const C * name)
		{
			return iterator (tree_find (name, tree.first_vector));
		}
		
		iterator partial_find (const C * name)
		{
		...
		Dictionary<T,C> ()
		{
			...
			tree.first_vector=(t_name_vector*)calloc (sizeof(C), sizeof(t_name_vector));
	...
	template<typename T,typename C>
		typename Dictionary<T,C>::t_name_record *
			Dictionary<T,C>::tree_add (const C * name)
	{
		using namespace std;
		int current = 0;
		...
		while (1)
		{
			...
			else if (current_vector->record->type==T_RECORD)
			{
				...
				{   /*name differs: must reorganize structure.*/
					...
					new_vector =
							(t_name_vector*)calloc (sizeof(C), sizeof(t_name_vector));
					if (!new_vector)
						throw bad_alloc ();
		...
		abort(); /*function shouldn't go here*/
	}//tree_add
	
	template<typename T,typename C>
		typename Dictionary<T,C>::t_name_record *
			Dictionary<T,C>::tree_partial_find 
				(const C * name,	t_name_vector * current_vector, 
	...
	template<typename T,typename C>
		typename Dictionary<T,C>::t_name_record *
			Dictionary<T,C>::tree_find (const C * name,
	...
	template<typename T,typename C>
		typename Dictionary<T,C>::t_name_record *
			Dictionary<T,C>::tree_find_next (t_name_record * r)
	{
		size_t i = 0;
		do
		{			
	...
	template<typename T,typename C>
		void Dictionary<T,C>::tree_erase_vector (t_name_vector * v)
	{
